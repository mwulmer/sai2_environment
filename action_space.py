...
class RobotAction(object):
    def __init__(self, action_space = None, isotropic_gains = True):
        ...
        self.dimensions = {
        ActionSpace.NONE: None,
        ActionSpace.ABS_JOINT_POSITION_DYN_DECOUP:  Box(low=np.concatenate((self._min_joint_position, self._min_kp)), 
                                                        high=np.concatenate((self._max_joint_position, self._max_kp)), 
                                                        dtype=np.float32), 
        ActionSpace.DELTA_JOINT_POSITION_DYN_DECOUP:Box(low=np.concatenate((self._min_joint_position_delta, self._min_kp_delta)), 
                                                        high=np.concatenate((self._max_joint_position_delta, self._max_kp_delta)), 
                                                        dtype=np.float32), 
        ActionSpace.ABS_JOINT_POSITION_IMPEDANCE:   Box(low=np.concatenate((self._min_joint_position, self._min_stiffness)), 
                                                        high=np.concatenate((self._max_joint_position, self._max_stiffness)), 
                                                        dtype=np.float32), 
        ActionSpace.DELTA_JOINT_POSITION_IMPEDANCE: Box(low=np.concatenate((self._min_joint_position_delta, self._min_stiffness_delta)), 
                                                        high=np.concatenate((self._max_joint_position_delta, self._max_stiffness_delta)), 
                                                        dtype=np.float32), 
        ActionSpace.ABS_JOINT_TORQUE:               Box(low=np.zeros((7,)), 
                                                        high=self._max_joint_torques, 
                                                        dtype=np.float32), 
        ActionSpace.DELTA_JOINT_TORQUE:             Box(low=-0.1*np.ones((7,)),
                                                        high=0.1*np.zeros((7,)),
                                                        dtype=np.float32), 
        ActionSpace.ABS_EE_POSE_DYN_DECOUP:         Box(low=np.concatenate((np.full((7,), -np.inf), self._min_kp)), 
                                                        high=np.concatenate((np.full((7,), -np.inf), self._max_kp)), 
                                                        dtype=np.float32), 
        ActionSpace.DELTA_EE_POSE_DYN_DECOUP:       Box(low=np.concatenate((np.full((7,), -np.inf), self._min_kp_delta)), 
                                                        high=np.concatenate((np.full((7,), -np.inf), self._max_kp_delta)), 
                                                        dtype=np.float32), 
        ActionSpace.ABS_EE_POSE_IMPEDANCE:         Box(low=np.concatenate((np.full((7,), -np.inf), self._min_stiffness)), 
                                                        high=np.concatenate((np.full((7,), -np.inf), self._max_stiffness)), 
                                                        dtype=np.float32), 
        ActionSpace.DELTA_EE_POSE_IMPEDANCE:        Box(low=np.concatenate((np.full((7,), -np.inf), self._min_stiffness_delta)), 
                                                        high=np.concatenate((np.full((7,), -np.inf), self._max_stiffness_delta)), 
                                                        dtype=np.float32), 
        }
