...
class PushPuckEasy(Task):
    def __init__(
        ...
        if simulation:
            ...
            self.lambda1 = 2 / self.euclidean_distance(
                self.get_ee_position(), reach_pos
            )

            self.lambda2 = 2 / self.euclidean_distance(
                self.get_ee_position(), self.goal_position
            )
        ...
        if not reached:
            ...
            reward = self.cr * (
                ...
                - (
                    np.tanh(
                        self.lambda1
                        * self.euclidean_distance(reach_pos, ee_pos)
                    )
                )
            )

        elif self.is_in_goal(puck_pos):
            # print("Goal", end="\r")
            reward = 10
        else:
            # print("Reached", end="\r")
            reward = self.cp * (
                ...
                - (
                    np.tanh(
                        self.lambda2
                        * self.euclidean_distance(goal_pos, puck_pos)
                    )
                )
            )
    ...
    def plan_optimal_trajectory(self):
        ...
        reach_pos = puck_pos[:2] + 0.1 * direction
        reach_pos = np.append(reach_pos, [0.05])
        ...
        a7 = np.array(
            [
                ...
                self.goal_position[1] - np.sign(self.goal_position[1]) * 0.05,
                0.05,
                0,
                0,
            ]
