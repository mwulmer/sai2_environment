class RedisKeys(object):
    def __init__(self, simulation=True):
        ...
        self.ACTION_COMPLETE_KEY = (
            "sai2::ReinforcementLearning::action_complete"
        )
        ...
        if simulation:
            self.JOINT_TORQUES_COMMANDED_KEY = (
                "sai2::PandaApplication::actuators::fgc"
            )
            self.JOINT_ANGLES_KEY = "sai2::PandaApplication::sensors::q"
            self.JOINT_VELOCITIES_KEY = "sai2::PandaApplication::sensors::dq"

            self.SENSED_CONTACT_KEY = (
                "sai2::PandaApplication::sensors::contact"
            )

            self.HARD_RESET_SIMULATOR_KEY = (
                "sai2::ReinforcementLearning::hard_reset_simulator"
            )

        else:
            self.JOINT_TORQUES_COMMANDED_KEY = (
                "sai2::FrankaPanda::actuators::fgc"
            )
            self.JOINT_ANGLES_KEY = "sai2::FrankaPanda::sensors::q"
            self.JOINT_VELOCITIES_KEY = "sai2::FrankaPanda::sensors::dq"
            self.SENSED_CONTACT_KEY = (
                "sai2::FrankaPanda::sensors::model::contact"
            )
            self.MASSMATRIX_KEY = (
                "sai2::FrankaPanda::sensors::model::massmatrix"
            )
            self.CORIOLIS_KEY = "sai2::FrankaPanda::sensors::model::coriolis"
            self.ROBOT_GRAVITY_KEY = (
                "sai2::FrankaPanda::sensors::model::robot_gravity"
            )
