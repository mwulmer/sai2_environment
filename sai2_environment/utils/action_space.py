...
class TaskSpaceAction(RobotAction):
    def __init__(
        ...
        (
            ...
            self.value_type,
            self.control_type,
        ) = self.decode_action_space(self.action_space_enum)

        # either generate full quaternion or rotate around the specified axis
        self.pose_dim = (
            7 if sum(rotation_axis) == 3 else 7 - 4 + sum(rotation_axis)
        )
        ...
        if self.value_type == 1:  # absolute position
            ...
            if self.control_type == 1:  # dynamic decoupling
                ...
                kp_min = (
                    ...
                    else np.array([0, 0, 0, 5, 5, 5])
                )
                kp_max = (
                    np.array([500, 10])
                    if self.isotropic_gains
                    else np.array([500, 500, 500, 10, 10, 10])
                )
